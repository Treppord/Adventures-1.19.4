...
public class C3POEntity extends VillagerEntity implements GeoEntity{
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public C3POEntity(EntityType<? extends C3POEntity> entityType, World world) {
    ...
    private <T extends GeoAnimatable> PlayState walkingPredicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.c3po.walk", Animation.LoopType.LOOP));
        } else {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.c3po.idle", Animation.LoopType.LOOP));
        }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> state) {
        if(state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.c3po.walk", Animation.LoopType.LOOP));
        } else {

        state.getController().setAnimation(RawAnimation.begin().then("animation.c3po.idle", Animation.LoopType.LOOP));
        }
        return PlayState.CONTINUE;
    }
