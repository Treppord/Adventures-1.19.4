...
public class DagobahItem extends Item {
    public DagobahItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient) {
            ...
            try {
                String command = "cdim pswg-adventures:dagobah";
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if(Screen.hasShiftDown()) {
            tooltip.add(Text.literal("Dagobah").formatted(Formatting.YELLOW));
        } else {
