...
public class FabricatorScreen extends HandledScreen<FabricatorScreenHandler>
{
    ...
    public static final Identifier TEXTURE = TestAddon.id("textures/gui/smithing_anvil_v3.png");
    ...
    private void renderRecipeBackground(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int indexOfLastVisibleRecipe)
    {
        for(int i = this.scrollOffset; i < indexOfLastVisibleRecipe && i < this.handler.getAvailableRecipeCount(); i++)
        {
            ...
            final int arrowOffset = (SCROLLBAR_THUMB_WIDTH + SCROLLBAR_THUMB_WIDTH) + (!canAfford ? SCROLLBAR_THUMB_WIDTH : 0);
    ...
    private void renderRecipeIcons(int x, int y, int indexOfLastVisibleRecipe)
    {
        ...
        MatrixStack matrixStack = new MatrixStack(); // create a new MatrixStack
        ...
        for(int i = this.scrollOffset; i < indexOfLastVisibleRecipe && i < this.handler.getAvailableRecipeCount(); i++)
        {
            ...
            //push the matrix stack before rendering output and material items
            matrixStack.push();

            //translate the matrix stack to the position of the output item
            matrixStack.translate(recipeButtonX + 46, recipeButtonY, 0);

            //render output item with the transformed matrix stack
            final ItemStack outputStack = list.get(i).craft(this.handler.input);
            this.client.getItemRenderer().renderInGuiWithOverrides(matrixStack, outputStack, recipeButtonX + 46, recipeButtonY);
            this.client.getItemRenderer().renderGuiItemOverlay(matrixStack, this.textRenderer, outputStack, recipeButtonX + 46, recipeButtonY);

            //pop the matrix stack after rendering the output item
            matrixStack.pop();

            //push the matrix stack before rendering the material item
            matrixStack.push();

            //translate the matrix stack to the position of the material item
            matrixStack.translate(recipeButtonX + 2, recipeButtonY - 1, 0);

            //render material item with the transformed matrix stack
            final ItemStack materialStack = this.handler.materialSlot.getStack().copy();
            materialStack.setCount(list.get(i).getMaterialCost());
            this.client.getItemRenderer().renderInGuiWithOverrides(matrixStack, materialStack, recipeButtonX + 2, recipeButtonY - 1);
            this.client.getItemRenderer().renderGuiItemOverlay(matrixStack, this.textRenderer, materialStack, recipeButtonX + 2, recipeButtonY - 1, materialStack.getCount() == 1 ? "1" : null);

            //pop the matrix stack after rendering the material item
            matrixStack.pop();
        }
    }
