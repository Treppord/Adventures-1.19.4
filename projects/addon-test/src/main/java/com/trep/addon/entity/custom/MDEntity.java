...
public class MDEntity extends OcelotEntity implements GeoEntity{
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public MDEntity(EntityType<? extends MDEntity> entityType, World world) {
    ...
    private <T extends GeoAnimatable> PlayState walkingPredicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.mouse_droid.walk", Animation.LoopType.LOOP));
        } else {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.mouse_droid.idle", Animation.LoopType.LOOP));
        }
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> state) {
        if(state.isMoving()) {
            state.getController().setAnimation(RawAnimation.begin().then("animation.mouse_droid.walk", Animation.LoopType.LOOP));
        } else {

        state.getController().setAnimation(RawAnimation.begin().then("animation.mouse_droid.idle", Animation.LoopType.LOOP));
        }
        return PlayState.CONTINUE;
    }
